...
from models import delete_item, update_item
...
def apply_custom_css():
    ...
    <style>
        ...
        /* Stat cards - ensure consistent height */
        .stat-card {
            padding: 1.5rem;
            border-radius: 0.5rem;
            margin: 0.5rem 0;
        ...
        @media (max-width: 768px) {
            ...
            .mobile-card-view .radio-item-header {
                font-weight: bold;
                border-bottom: 1px solid #eee;
                padding-bottom: 5px;
                margin-bottom: 5px;
            ...
            .mobile-card-view .radio-item-detail {
                display: flex;
                justify-content: space-between;
                padding: 2px 0;
            }

            .mobile-card-view .radio-item-label {
                font-weight: 500;
                color: #666;
            }
        }
...
def render_stats_cards(items):
    ...
    with col5:
        conditions = items["condition"].value_counts()
        poor_condition = conditions.get("Poor", 0)
        if poor_condition > 0:
            st.markdown(
                f'<div class="stat-card" style="background-color: #dc3545; border-left: 4px solid #ff0000;">'
                f"<h2 style='color: white;'>{poor_condition}</h2>"
                f"<p style='color: white; font-weight: bold;'>⚠️ Poor Condition</p>"
                f"<p style='font-size: 0.8rem; color: #ffe6e6; margin: 0;'>Needs Attention</p>"
                "</div>",
                unsafe_allow_html=True,
            )
        else:
            st.markdown(
                f'<div class="stat-card" style="background-color: #28a745;">'
                f"<h2>{poor_condition}</h2>"
                f"<p>Poor Condition</p>"
                "</div>",
                unsafe_allow_html=True,
            )
...
def render_radio_gallery():
    """Render a gallery of radio equipment images"""
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.subheader("📡 Radio Equipment Gallery")
    ...
    radio_images = [
        ("radio1.png", "RadioTrack System Logo"),
        ("radio2.jpg", "Portable Radio Equipment"),
        ("radio4.jpg", "Mobile Radio Installation"),
    ]

    descriptions = {
        "radio1.png": "RadioTrack - Department of Corrections Radio Management System",
        "radio2.jpg": "Various portable radio models used in corrections facilities",
        "radio4.jpg": "Mobile radio installations in transport vehicles",
    }

    for i, (img_file, title) in enumerate(radio_images):
        if i % 3 == 0:
            with col1:
                img_path = Path(__file__).parent / "static" / img_file
                if img_path.exists():
...
def render_inventory_table(items):
    for _, item in items.iterrows():
        ...
        with cols[0]:
            # Add red alert banner for poor condition items
            if item["condition"] == "Poor":
                st.markdown(
                    f"""
                    <div style="background-color: #f8d7da; border: 1px solid #f5c6cb; border-radius: 5px; padding: 8px; margin-bottom: 10px;">
                        <div style="display: flex; align-items: center; color: #721c24;">
                            <span style="font-size: 1.2rem; margin-right: 8px;">⚠️</span>
                            <strong>URGENT: This item is in poor condition and needs immediate attention!</strong>
                        </div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                        ...
                        with col1:
                            if st.button(
                                ...
                                if delete_item(item["id"]):
                                    st.success("✅ Radio deleted successfully!")
                                    time.sleep(1)
                                    st.rerun()
                                else:
                                    st.error("❌ Failed to delete radio.")
                        with col2:
                            if st.button("Cancel", key=f"cancel_delete_{item['id']}"):
                                st.session_state[delete_confirm_key] = False
                                st.rerun()
                else:
                    st.error("❌ Only corrections supervisors can delete radios.")
                    time.sleep(2)
                    st.rerun()
...
def render_login_form():
    ...
    st.markdown(
        '<h1 class="main-header">RadioTrack - MCC Radio Inventory System</h1>',
        unsafe_allow_html=True,
    )
...
def render_sidebar_navigation(user_role):
    with st.sidebar:
        # Add RadioTrack logo at the top
        img_path = Path(__file__).parent / "static" / "radio1.png"
        st.markdown(
            """
        ...
        st.markdown(
            "<h3 style='text-align: center; color: #1E88E5;'>RadioTrack - MCC Radio Inventory</h3>",
            unsafe_allow_html=True,
        )
        ...
        if st.button("📊 Dashboard", use_container_width=True):
            ...
            st.rerun()
        if st.button("📡 Radio Inventory", use_container_width=True):
            st.session_state.view = "inventory"
            st.rerun()
        if st.button("➕ Add Radio", use_container_width=True):
            st.session_state.view = "add"
            st.rerun()

        # Only show Admin Dashboard to corrections supervisor
        if user_role == "corrections_supervisor":
...
def render_system_stats():
    """Render system resource statistics including CPU, memory, and disk usage"""
    st.markdown('<div class="card">', unsafe_allow_html=True)
