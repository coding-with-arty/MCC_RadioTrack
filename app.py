...
# Set page config
st.set_page_config(
    page_title="RadioTrack - MCC Radio Inventory",
    page_icon="📡",
    layout="wide",
...
# Set background
bg_path = Path(__file__).parent / "static/1.jpg"
if bg_path.exists():
...
def check_session_timeout():
    ...
    timeout_seconds = SESSION_EXPIRY_HOURS * 3600
...
def force_password_change_screen():
    ...
    if not st.session_state.user.get("password_change_required", False):
        ...
        if st.session_state.user["role"] == "corrections_supervisor":
        ...
        if submit:
            if not new_password or new_password != confirm_password:
                ...
                if not is_valid:
                    ...
                    # For forced password change, skip old password verification
                    success, message = change_password(
                        st.session_state.user["username"],
                        None,  # Skip old password verification for forced changes
                        new_password,
                    )
...
def show_login_page():
    ...
    st.title("RadioTrack - MCC Radio Inventory System")
...
def handle_login(username, password):
    ...
    if user_data.get("role") == "corrections_supervisor":
        st.session_state.view = "admin_dashboard"
...
def dashboard_page():
    ...
    # Check for poor condition items and show alert
    items = get_items()
...
def admin_dashboard_page():
    ...
    if st.session_state.user["role"] != "corrections_supervisor":
        st.error("Access denied. Only Corrections Supervisor can access this page.")
        st.session_state.view = "dashboard"
        st.rerun()
    ...
    with tab1:
        ...
        with post_form_col:
            ...
            with st.form("new_post_form"):
                post_content = st.text_area(
                    "Share an announcement with everyone:", height=150
                )
                submitted = st.form_submit_button(
    ...
    with tab2:
            ...
            with st.form("add_employee_form"):
                ...
                with col1:
                    ...
                    user_role = st.selectbox(
                        "Role",
                        ["employee", "corrections_supervisor"],
                        format_func=lambda x: {
                            "employee": "Employee",
                            "corrections_supervisor": "Corrections Supervisor",
                        }[x],
                    )
        ...
        from auth import approve_user, get_pending_approvals
                ...
                with cols[2]:
                    if st.button(
                        ...
                        if (
                            ...
                            st.error(
                                "❌ Only corrections supervisors can delete radios"
                            )
                            time.sleep(1)
                            st.rerun()
...
def add_item_page():
    ...
    st.title("Add New Radio")
    st.subheader("Add a new radio to the inventory")
...
def edit_item_page():
    ...
    if "edit_item_id" in st.session_state and st.session_state.edit_item_id is not None:
        ...
        st.title("Edit Radio")
        st.subheader("Edit an existing radio in the inventory")
...
def is_production():
    ...
    # This is a placeholder function - in app this would check
            ...
            if st.session_state.user.get("password_change_required", False):
                ...
                if st.session_state.view == "dashboard":
                    ...
                    if st.session_state.user.get("role") == "corrections_supervisor":
                        ...
                        print(f"   ❌ Access denied - redirecting to dashboard")
                        st.session_state.view = "dashboard"
                        st.rerun()
